...
function listProjects() {
  return loadHistory().map((entry) => ({
    ...
    language: entry.language || 'node',
    project_type: entry.project_type || 'security_tool',
  }));
}
...
function getToolPath(slug) {
  ...
  const toolFile = project && project.language === 'python' ? 'main.py' : 'index.js';
  return path.join(DATA_DIR, slug, 'tool', toolFile);
}
